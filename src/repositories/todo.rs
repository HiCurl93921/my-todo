...
mock! {
    ...
    impl Clone for TestTodoRepository {
        fn clone(&self) -> Self;
    }
}
...
mod test {
    ...
    async fn crud_scenario() {
        ...
        let todo = repository
            .find(created.id)
            .await
            .expect("[find] returnd Err");
        assert_eq!(created, todo);

        // all
        let todos = repository.all().await.expect("[all] retuned Err");
        let todo = todos.first().unwrap();
        assert_eq!(created, *todo);

        // update
        let updated_text = "[crud scenario] update text";
        let todo = repository
            .update(
                todo.id,
                UpdateTodo {
